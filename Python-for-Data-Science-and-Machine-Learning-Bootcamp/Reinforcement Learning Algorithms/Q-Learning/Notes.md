...
- Exploration : The agent picks the action with highest Q-value with probability 1-E. this means agent uses it's current knowledge ot maximize rewards 
- Exploitation : With probability E, the agent picks a random action, exploring new possibilities to learn if there are better ways to get rewards. This allows the agent to discover new strategies and improve it's decision-making over time 
